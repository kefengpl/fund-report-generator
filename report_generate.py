...
def multi_fund_report(netval_path: str, index_path: str, enhanced_fund: bool, **kwargs):
    ...
    for idx in tqdm(range(funds_num)):
        generate_report(netval_data.iloc[:, idx], index_data, enhanced_fund, corp_names[idx], start_dates[idx],
                        fund_name = fund_names[idx], index_name = index_name)
...
def generate_word_indicator_tables(netval_data: pd.Series,  corp_name: str = "私募管理人", 
    ...
    # PART1：获取生成word所需要的数据[也就是三张表的数据]
    summary_indicator_table = np.r_[utils.dict_to_matrix(this_fund.summary_indicators()), 
                                    utils.dict_to_matrix(this_fund.all_recent_return())]
    rolling_quantile_table = utils.df_to_matrix(this_fund.get_rolling_quantile_dataframe())
    earning_probability_table = utils.df_to_matrix(this_fund.get_earning_probability())
    ...
    # 生成标题
    word_handler.add_text_content(f"1. {this_fund.fund_name}关键指标汇总", "title")
    word_handler.add_text_content("", "footnote")
    # 生成“收益风险指标”表格及其脚注
    ...
    # 生成标题
    word_handler.add_text_content(f"2. {this_fund.fund_name}滚动收益率分布", "title")
    word_handler.add_text_content("", "footnote")
    # 生成“收益风险指标”表格及其脚注
    ...
    # 生成标题
    word_handler.add_text_content(f"3. {this_fund.fund_name}收益概率统计", "title")
    word_handler.add_text_content("", "footnote")
    # 生成“收益风险指标”表格及其脚注
    ...
    # 保存文件并退出
    word_handler.close_and_save(this_fund.fund_name + '_指标汇总及滚动收益统计')
    # 打印警告信息
    print_warning_messages(this_fund.fund_name, this_fund.get_first_netval_date())
...
def print_warning_messages(fund_name: str, first_netval_date: date):
    ...
    warning_message2 = "警告信息2：由于前两个警告信息，将使得夏普比、卡玛比、索提诺比的结果可能会有偏差，这是正常现象，不代表出现了错误，一般情况下偏差会很小。"
    # 打印警告信息
