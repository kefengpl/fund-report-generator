...
class WordHandler:
    def __init__(self, path: str, visible: bool = True, 
                 chinese_font: str = "楷体", english_font: str = "Times New Roman"):
        """
        此类之作用在于操作 WORD 生成文字、段落、表格。

        Args:
            - path (str): 你希望写入word文档的绝对路径，该 word 文档需要是空文档
        ...
        self.this_doc = self.word_app.Documents.Open(path) # 打开我的文档 
        ...
        self.font_size = { # 字体在这里设定即可
            "title" : 12, # 一级标题：12号字
            "paragraph" :  10.5, # 二级标题：10.5号字
            "footnote" : 10.5 # 脚注字体大小
        }
        self.line_spacing = { # 设置行间距，起始有换算公式，例如：1.25倍行距 = 字体大小 * 1.25
            "title" : 13.5,
            "paragraph" : 15, 
            "footnote" : self.font_size["footnote"]
        }
    ...
    def add_picture(self, picture_path: str):
        ...
        #指定文件的完整路径
        picture_path = picture_path #r"C:\Users\3590\Desktop\自动化项目工程文件\image\图像测试.svg"
        #在当前的段落中插入图片
    ...
    def add_table(self, n_rows: int, n_cols: int, title_mode: str, text_array: np.ndarray):
        ...
        table_handler = wth.WordTableHandler(this_table, "sep")
