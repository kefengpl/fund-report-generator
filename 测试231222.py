...
hdr_rng=doc.Sections(1).Headers(constants.wdHeaderFooterFirstPage).Range #页眉位置
hdr_rng.InsertAfter ("这是页眉") #页眉内容
hdr_rng.Paragraphs.Alignment=1 #页眉对齐
ftr_rng=doc.Sections(1).Footers(constants.wdHeaderFooterFirstPage).Range #页脚位置
ftr_rng.Text=""
doc.Fields.Add(Range=ftr_rng,Type=constants.wdFieldPage) #插入页码
ftr_rng.Paragraphs.Alignment=1 #页码对齐
doc_range=doc.Range()
doc_range.InsertAfter('\r\n') #在word现有内容最后换行
text=doc.Paragraphs.Last #定位到最后一段
text.CharacterUnitFirstLineIndent=2 #段前空两个字符
text.Alignment=3 #文字两端对齐
text_range=text.Range
text_range.Text="这是一个新段落！" #段落内容
text_range.Font.Name="楷体" #段落字体
text_range.Font.Color=255 #字体颜色
text_range.Font.Size=20 #字体大小
