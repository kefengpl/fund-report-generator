...
def multi_fund_report_interface():
    ...
    netval_path = "data/聚宽对冲五号-净值数据.xlsx" # 净值数据表路径，可以包含多个基金，每个基金的数据是一列，列名是基金名称。不要出现任何冗余列。
    index_path = "data/聚宽对冲五号-指数数据.xlsx"  # 所有基金共用该指数数据表。指数数据表路径：① 非指增：允许添加多列指数数据；② 指增：只允许添加一列指数数据
    ...
    corp_names: list = ["聚宽投资"] # 从前往后按顺序匹配公司名称，使用方法与 start_dates 的使用完全一致
